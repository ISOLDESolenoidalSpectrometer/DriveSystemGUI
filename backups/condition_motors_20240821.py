...
def main():
    ...
    # AXIS_LIST = [5,6,7]
    # AXIS_NAMES = [ "Target_vertical", "Beam_blocker_horizontal", "Beam_blocker_vertical" ]
    AXIS_LIST = [7]
    AXIS_NAMES = [ "Target_vertical" ]

    # Output to Grafana
    g = Grafana( AXIS_LIST, AXIS_NAMES )
    g.start()

    # -------- EDIT BELOW HERE TO CHANGE HOW THE MOTORS MOVE -------- #
    cmt_axis = ConditionMotorsThread(AXIS_LIST[0], AXIS_NAMES[0], "" ) # Setup a thread to move the motors
    cmt_axis.set_speeds( 100, 1600 )     # First one is creep speed (slow speed), second one is slew speed (normal speed)
    cmt_axis.set_limits( -15600, 12000) # Limits of the range - this only works if axis has been datum'ed
    cmt_axis.set_grafana_location(g)    # Can ignore this
    cmt_axis.set_print()                # Print to console
    cmt_axis.start()                    # Start moving!

    # cmt_axis_6 = ConditionMotorsThread(AXIS_LIST[1], AXIS_NAMES[1], " "*20 )
    # cmt_axis_6.set_speeds( 100, 400 )
    # cmt_axis_6.set_limits( -14000, 13500)
    # cmt_axis_6.set_grafana_location(g)
    # cmt_axis_6.start()

    # cmt_axis_7 = ConditionMotorsThread(AXIS_LIST[2], AXIS_NAMES[2], " "*40 )
    # cmt_axis_7.set_speeds( 100, 400 )
    # cmt_axis_7.set_limits( -13600, 17000)
    # cmt_axis_7.set_grafana_location(g)
    # cmt_axis_7.start()
    ...
    # cmt_axis_6.is_running = False
    # cmt_axis_7.is_running = False
    g.is_running = False
