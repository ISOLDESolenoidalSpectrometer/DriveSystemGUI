...
# Frank's attempt at window
class BeamView(wx.Panel):
...
#		self.split_win = wx.SplitterWindow(panel)
#		self.vert_split = SplitVertically(self.split_win)
		#Defining the figure
		self.fig = plt.figure()
		...
		#Arrow who shows the distance for recoil to target
		self.arrow2=self.ax.annotate ('', (-100, 100), (100, 100), arrowprops={'arrowstyle':'<->'})
		self.distanceArrow2=self.ax.text(0,0,"")
		self.drawArrow2(20)	

		#Arrow who shows the distance for silicon to target for physics
		self.arrow3=self.ax.annotate ('', (-100, 100), (100, 100), arrowprops={'arrowstyle':'<->'})
	...
	def drawArrow4(self, blocker_target_dist):
		self.arrow4.remove()
	...
	def move1(self,moveDis):
		...
		self.drawArrow()
		self.drawArrow2() # Recoil distance arrow
		self.drawArrow3()
		self.changeText(1)
	...
	def updatePositions(self,pos):
		...
		self.silencer.set_x(self.array.get_x()+arrayW+arrayEdge_W)
...
class ControlView(wx.Panel):
	...
	def __init__(self,parent,matplotpanel,frame):
		...
		self.datum2Button.Bind(wx.EVT_BUTTON, self.datum2B)
		self.datum3Button = wx.Button(self, wx.ID_ANY, "Datum 3", (2*dis,sizeY-h),(dx,dy))
		...
		self.TargetPos = wx.StaticText(self, -1, "(1):", (fac_1*dis,disy-50))
		...
		# axis 2 ARRAY
		fac_2 = 1
		self.TargetPos = wx.StaticText(self, -1, "(2):", (fac_2*dis,disy-50))
		...
		self.TargetPos = wx.StaticText(self, -1, "(3):", (fac_3*dis,disy-50))
		...
		self.TargetPos = wx.StaticText(self, -1, "(4):", (fac_4*dis,disy-50))
		...
		self.TargetPos = wx.StaticText(self, -1, "(5):", (fac_5*dis,disy-50))
		...
		self.TargetPos = wx.StaticText(self, -1, "(6):", (fac_6*dis,disy-50))
		...
		self.TargetPos = wx.StaticText(self, -1, "(7):", (fac_7*dis,disy-50))
...
class DriveSystemGUI(wx.Frame):
	...
	def InitUI(self):
		...
		self.split_right_win = wx.SplitterWindow(self)
		self.right_split = BeamView(self.split_right_win)
		self.split_win.SplitVertically(bottom_split,right_split)
...
class SettingsWindow(wx.Frame):
	...
	def InitUI(self):
		...
		det0label = wx.StaticText(self.panel, wx.ID_ANY, 'ZD')
